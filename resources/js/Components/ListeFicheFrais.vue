<template>
  ...
  <div class="flex flex-row-reverse">
    <main class="flex-1 justify-center h-full overflow-visible">
      <div class="px-4 pb-3 w-full sm:px-6 lg:px-8 mx-auto">
        <!-- Main area -->
        <div class="">
          <div
          ...
          <p class="my-2 text-sm text-gray-500">
            Liste pour les 12 derniers mois
          </p>
        </div>
        <div>
          <ul role="list" class="divide-y divide-white/5">
            ...
            <li
              ...
              :key="ficheFrais.id"
              class="overflow-hidden rounded-md bg-white my-3 shadow cursor-pointer"
            >
              ...
              <Link
                ...
                "
                class="flex items-center justify-between px-6 py-4 gap-x-3 sm:gap-x-0"
              >
...
const filteredFicheFrais = listeFicheFrais
  .filter((f) => {
    ...
    // Inclure les fiches qui ont des frais hors forfait validés ou frais forfaitisés validés, ou celles du mois en cours
    return (
      (Array.isArray(f.frais_hors_forfait) &&
        f.frais_hors_forfait.some(
          (frais) => frais.etat === "validé" || frais.etat === "en attente"
        )) || // Filtre les frais hors forfait validés
      (Array.isArray(f.frais_forfait) &&
        f.frais_forfait.some(
          (frais) => frais.etat === "validé" || frais.etat === "en attente"
        )) || // Filtre les frais forfaitisés validés
      ficheMonth === currentMonth // Inclure les fiches du mois en cours
    );
  })
  .map((f) => {
    ...
    // Calcul du montant total des frais hors forfait et forfaitisés validés uniquement
    f.montantTotal = 0;

    // Calcul des frais hors forfait validés
    ...
    // Calcul des frais forfaitisés validés uniquement
